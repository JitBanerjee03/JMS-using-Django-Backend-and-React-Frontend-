...
const ApproveReviewers = () => {
    const { reviewers, fetchReviewers, areaEditor,fetchApprovedReviewers } = useContext(contextProviderDeclare);
    ...
    const unapprovedReviewers = reviewers?.filter(reviewer => !reviewer.is_approved) || [];
    ...
    const filteredReviewers = unapprovedReviewers?.filter(reviewer => {
        const matchesSearch = 
    ...
    const handleApprove = async (reviewerId, approve) => {
        setLoading(prev => ({ ...prev, [reviewerId]: true }));
        setError(null);
        setSuccess(null);

        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_DJANGO_URL}/reviewer/approve/${reviewerId}/`, {
                method: 'PATCH',
                headers: {
            ...
            setSuccess(`Reviewer ${approve ? 'approved' : 'rejected'} successfully`);
        } catch (err) {
    ...
    return (
        <div className="container-fluid p-3">
            <h4 className="mb-4">Reviewer Approval Queue</h4>
                                    ...
                                    <div className="d-flex gap-2">
                                        ...
                                        {hasMatchingSubjectArea(reviewer) && (
                                            <Button 
                                                variant="success" 
                                                size="sm"
                                                onClick={() => handleApprove(reviewer.id, true)}
                        ...
                        <tr>
                            <td colSpan="7" className="text-center">
                                {unapprovedReviewers.length === 0 
                                    ? "All reviewers have been approved" 
